...
interface StoryDao {
    ...
    @Query("SELECT * FROM userreadsstory " +
            "JOIN user AS author ON userreadsstory.user = author.uuid" +
            "JOIN story ON userreadsstory.story = story.uuid" +
    ...
    @Query("SELECT * FROM userratesstory " +
            "JOIN story ON userratesstory.story = story.uuid" +
            "WHERE story.storyTitle LIKE :storyTitle ")
    fun getStoryReviews(storyTitle: String): List<UserRatesStory>

    @Query("UPDATE userratesstory" +
            "JOIN user ON userratesstory.user = user.uuid" +
            "JOIN story ON userratesstory.story = story.uuid" +
